        ...
        if ($username != '' && $password!='') { 
            $data = $this->m_login->check($username,$password);
             if($data == 'False'){
                ...
                $random = $rand .'_'.  $date ; 
                 $dataset = $this->m_login->set_token($data[0]->id,$random); 
...
//         $students_id = $request['students_id'];
    ...
    function app_attendance_add_post(){
        ...
        if ($token != '') {
            $users = $this->m_login->get_users($token);
            $attend  =  json_decode($attendance,true);
            foreach($attend as $attend){
                $att  = $this->m_attendances->attendance_add_app($class,$section,$date,$teacher_id,$attend['id'],$users[0]->school_id,$users[0]->id);
            }
            $d = explode('_',$token);
            $endDay = strtotime(date('Y/m/d H:i:s', strtotime('+1 day',strtotime($d[1]))));
